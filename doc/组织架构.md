...
| 添加节点       | 2 SQL                                    |
| 删除节点       | 不能删除非空节点，2 SQL                  |
| 移动节点       | path表数据移动， 2 SQL                   |
| 添加链接(Link) | 暂时用不到，支持复杂的组织架构可能会用到 |
| 移除链接(Link) | 暂时用不到，支持复杂的组织架构可能会用到 |
...
| :--:                       | :--:                             |
| `org`                      | 存储公司信息                     |
| `org_${org_id}_department` | 存储某个公司的部门（子公司）信息 |
| `org_${org_id}_position`   | 存储某个公司的岗位分类信息       |
| `org_${org_id}_member`     | 存储某个公司的员工信息表         |
| `org_${org_id}_tree`       | 存储某个公司的隶属关系           |
| `org_${org_id}_manager`    | 存储某个公司部门管理者           |
| `org_${org_id}_path`       | 存储某个公司的路径检索信息       |

### 剥离业务数据

识别业务数据的方法：

+ 业务自己产生数据类型，非第三方系统（`HR系统`）导入的数据。
+ 考虑多个下游业务使用的场景，例如：IM业务，课程学习业务， HR系统(虚拟)三个使用组织架构的业务都需要的数据。
+ 展示类数据大部分为业务数据。
